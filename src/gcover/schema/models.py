...
class ESRISchema:
    ...
    def get_coded_values_by_source(self) -> Dict[str, Dict[Union[str, int], str]]:
        ...
        for domain_name, domain in self.coded_domains.items():
            result[domain_name] = {
                cv.code: cv.name for cv in domain.coded_values
            }

        # Add subtypes
        for subtype_name, subtype in self.subtypes.items():
            result[subtype_name] = {
                sv.code: sv.name for sv in subtype.subtypes
            }

        return result

    def find_coded_value(self, code: Union[str, int], source: Optional[str] = None) -> List[Dict[str, Any]]:
        """
